...
// === Middlewares ===
app.use(cors());
app.use(express.json()); // ¡Importante! Para parsear el cuerpo JSON
...
app.post('/api/login', (req, res) => {
  ...
  // Verificación explícita por si acaso req.body es undefined
  if (!req.body) {
  ...
  // 2. Validación básica: Verificar que email y password existen
  if (!email || !password) {
  ...
  // 3. Simulación de Autenticación (¡IMPORTANTE: Esto es temporal!)
...
// === Iniciar el Servidor ===
// Esta debe ser la ÚLTIMA parte del archivo (excepto exports si los hubiera)
app.listen(PORT, () => {
